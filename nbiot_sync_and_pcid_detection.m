...
% 
% try
%     % 优先尝试使用NB-IoT专用函数生成参考信号
%     if exist('lteNBPSS', 'file') && exist('lteNBSSS', 'file')
%         % 使用NB-IoT专用函数
%         pssSeq = lteNBPSS(detectedPCID);
%         sssSeq = lteNBSSS(detectedPCID);
%         fprintf('使用NB-IoT专用函数生成参考信号\n');
%         mappingNote = '直接使用NB-IoT PCID';
%     else
%         % 回退方案：映射到LTE小区ID
%         fprintf('警告：lteNBPSS/lteNBSSS不可用，使用LTE函数替代\n');
% 
%         % NB-IoT PCID到LTE小区ID的映射
...
% 
%         mappingNote = sprintf('映射PCID %d→%d', detectedPCID, lteCellId);
%         fprintf('使用LTE函数生成参考信号\n');
